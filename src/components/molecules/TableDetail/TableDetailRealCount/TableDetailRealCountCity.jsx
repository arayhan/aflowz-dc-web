...
export const TableDetailRealCountCity = ({ isLoading, realcountCityData }) => {
		...
		() => [
			...
			},
			{
				...
				Cell: (row) => {
					...
					return <div className="text-gray-400">{data?.city_name ?? '-'}</div>;
				}
			},
			{
				Header: 'Jumlah TPS',
	...
	return (
		<Table
			loading={isLoading}
			columns={columns}
			onClickRow={(data) => navigate(`/realcount/city/${data.city_id}`)}
			data={realcountCityData}
		/>
	);
};
