...
export const TableDetailRealCountDistrict = ({ isLoading, realcountDistrictData }) => {
		...
		() => [
			...
			},
			{
				...
				Cell: (row) => {
					...
					return <div className="text-gray-400">{data?.district_name ?? '-'}</div>;
				}
			},
			{
				Header: 'Jumlah TPS',
	...
	return (
		<Table
			loading={isLoading}
			columns={columns}
			onClickRow={(data) => navigate(`/realcount/district/${data.district_id}`)}
			data={realcountDistrictData}
		/>
	);
};
