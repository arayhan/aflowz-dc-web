...
export const ModalUploadSheetAnonymousData = ({ onClose }) => {
	const navigate = useNavigate();
	const { processingBulkUploadAnonymouseData, bulkUploadAnonymouseData } = usePartnerStore();
		...
		if (file) {
			...
			reader.onload = (event) => {
				...
				if (json.length > 0) {
					...
					bulkUploadAnonymouseData(params, ({ payload, success }) => {
						if (success) {
							const queryParams = {
								is_no_nik_number: true,
								is_no_nisn_number: true,
