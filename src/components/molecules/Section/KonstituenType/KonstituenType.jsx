            ...
            <div>
                ...
                <div className="container">
                    <div className="overflow-x-auto pb-4 md:pb-6 flex 2xl:justify-center gap-3">
                        <button
                            className={`w-[200px] min-w-[200px] lg:w-[250px] lg:min-w-[250px] flex items-center text-left border px-2 py-2 rounded-md cursor-pointer space-x-2 ${selectedKonstituen === 'sekolah'
                                ? 'bg-primary-500  border-primary-500 hover:bg-primary-500 text-white'
                                : 'text-gray-400 hover:bg-gray-100'
                                }`}
                            onClick={
                                selectedKonstituen === 'sekolah' ?
                                    () => { onSelectedType(null); setSelectedKonstituen(''); }
                                    :
                                    () => { onSelectedType({ konstituen_type: 'sekolah' }); setSelectedKonstituen('sekolah'); }
                            }
                        >
                        ...
                        <button
                            className={`w-[200px] min-w-[200px] lg:w-[250px] lg:min-w-[250px] flex items-center text-left border px-2 py-2 rounded-md cursor-pointer space-x-2 ${selectedKonstituen === 'kampus'
                                ? 'bg-primary-500  border-primary-500 hover:bg-primary-500 text-white'
                                : 'text-gray-400 hover:bg-gray-100'
                                }`}
                            onClick={
                                selectedKonstituen === 'kampus' ?
                                    () => { onSelectedType(null); setSelectedKonstituen(''); }
                                    :
                                    () => { onSelectedType({ konstituen_type: 'kampus' }); setSelectedKonstituen('kampus'); }
                            }
                        >
                            <img className="w-10" src={require('@/images/icons/box.svg').default} alt="" />
